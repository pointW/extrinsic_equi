...
from networks.equivariant_sac_net import EquivariantSACActorDihedralAllInv, EquivariantSACCriticDihedralAllInv
...
from networks.equivariant_sac_net import EquivariantSACActorDihedralLatentIn, EquivariantSACCriticDihedralLatentIn, NonEquivariantEncConv, NonEquivariantEncFC, NonEquivariantEncSSM
...
def createAgent(test=False):
    ...
    if alg == 'dqn_fac':
        ...
        if obs_type == 'pixel':
            ...
            elif model == 'equi_both_d_bn':
                actor = EquivariantSACActorDihedralBN((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
                critic = EquivariantSACCriticDihedralBN((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
            ...
            elif model == 'equi_both_d_res16':
                actor = EquivariantSACActorDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res16').to(device)
                critic = EquivariantSACCriticDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res16').to(device)
            elif model == 'equi_both_d_res19':
                actor = EquivariantSACActorDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res19').to(device)
                critic = EquivariantSACCriticDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res19').to(device)
            elif model == 'equi_both_d_res31':
                actor = EquivariantSACActorDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res31').to(device)
                critic = EquivariantSACCriticDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n, backbone='res31').to(device)
            elif model == 'equi_d_actor':
                actor = EquivariantSACActorDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
                critic = SACCritic((obs_channel, crop_size, crop_size), len(action_sequence), ssm=True).to(device)
            elif model == 'equi_d_critic':
                actor = SACGaussianPolicy((obs_channel, crop_size, crop_size), len(action_sequence), ssm=True).to(device)
                critic = EquivariantSACCriticDihedral((obs_channel, crop_size, crop_size), len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
    ...
    elif alg in ['sac_share_enc', 'sac_share_enc_reg']:
        sac_lr = (actor_lr, critic_lr)
        if model == 'equi_both_d':
            enc = EquivariantEncoder128Dihedral(obs_channel, n_hidden, initialize, equi_n).to(device)
        ...
        elif model == 'equi_both_d_w_enc' or model == 'equi_both_d_w_enc_conv':
            enc = NonEquivariantEncConv((obs_channel, crop_size, crop_size), n_hidden, equi_n).to(device)
            actor = EquivariantSACActorDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
            critic = EquivariantSACCriticDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
        elif model == 'equi_both_d_w_enc_fc':
            enc = NonEquivariantEncFC((obs_channel, crop_size, crop_size), n_hidden, equi_n).to(device)
            actor = EquivariantSACActorDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
            critic = EquivariantSACCriticDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
        elif model == 'equi_both_d_w_enc_ssm':
            enc = NonEquivariantEncSSM((obs_channel, crop_size, crop_size), n_hidden, equi_n).to(device)
            actor = EquivariantSACActorDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
            critic = EquivariantSACCriticDihedralLatentIn(len(action_sequence), n_hidden=n_hidden, initialize=initialize, N=equi_n).to(device)
        else:
            raise NotImplementedError
